...
class RealityTwin3DAnalysisTool {
  ...
  /**
   * 初始化工具
   * @param {Object} externalViewer - 外部传入的viewer实例（可选）
   */
  init(externalViewer = null) {
    // 设置全局网络请求拦截器，阻止不必要的在线服务请求
    this.setupNetworkInterceptor()

    // 初始化组件
    this.initializeComponents(externalViewer)
  ...
  setupNetworkInterceptor() {
    ...
    const shouldAllow = (url) => {
      ...
      // 允许SuperMap3D的必要服务请求
      if (
        urlStr.includes('supermapol.com') &&
  ...
  fixPermissionsPolicyViolation() {
    try {
      ...
      document.removeEventListener = function (type, listener, options) {
        ...
        return originalDocRemoveEventListener.call(
          this,
          type,
          listener,
          options,
   ...
   * 初始化组件
   */
  initializeComponents(externalViewer = null) {
    // 如果传入了外部viewer，则使用外部viewer
    if (externalViewer) {
      this.viewer = externalViewer
      this.scene = externalViewer.scene
      console.log('✅ 使用外部传入的viewer实例')
    } else {
      this.initViewer()
    }
    ...
    setTimeout(() => {
      ...
      // 初始化数字孪生分析模块
      this.digitalTwinAnalysis = new DigitalTwinAnalysis(
        this.viewer,
        this.scene,
      )
      ...
      // 初始化智能分析助手
      try {
        this.intelligentAssistant = new IntelligentAnalysisAssistant(
          this.viewer,
          this.scene,
        )
        console.log('[Main] 智能分析助手初始化成功')
      } catch (error) {
        console.error('❌ 智能分析助手初始化失败:', error)
        this.intelligentAssistant = null
      }
  ...
  initViewer() {
    ...
    try {
      ...
      // 优化渲染设置
      if (this.scene.frustumCullingEnabled !== undefined) {
        this.scene.frustumCullingEnabled = true
      }

      // 减少渲染频率以提高性能
      this.scene.minimumZoomDelta = 0.1
      this.scene.maximumZoomDistance = 50000000

      // SuperMap3D Viewer创建成功
      ...
      const checkReady = () => {
        ...
        if (this.viewer && this.viewer.scene && this.viewer.scene.canvas) {
          ...
          if (
            ...
            try {
              ...
              // 成功创建并绑定screenSpaceEventHandler
            } catch (error) {
        ...
        if (isReady) {
          ...
          try {
            if (
              screenSpaceEventHandler &&
              typeof screenSpaceEventHandler.setInputAction === 'function'
            ) {
              // ScreenSpaceEventHandler功能验证通过
              resolve()
            } else {
              // ScreenSpaceEventHandler功能验证失败，继续等待
          ...
          if (canvas && !screenSpaceEventHandler) {
            try {
              this.screenSpaceEventHandler =
                new SuperMap3D.ScreenSpaceEventHandler(canvas)
  ...
  setupScene() {
    ...
    // 优化模型渲染
    this.scene.modelFadeOutEnabled = false
  ...
  setupCameraController() {
    try {
      ...
      controller.zoomEventTypes = [
        SuperMap3D.CameraEventType.WHEEL,
        SuperMap3D.CameraEventType.PINCH,
      ]
  ...
  bindEvents() {
    ...
    if (flyToModelsBtn) {
      console.log('[Main] 找到飞行到模型按钮，绑定事件')
    ...
    // 左侧面板折叠功能
    document
      .getElementById('toggleLeftPanel')
      .addEventListener('click', () => this.toggleLeftPanel())

    // 左侧面板展开按钮
    document
      .getElementById('expandLeftPanel')
      .addEventListener('click', () => this.expandLeftPanel())

    // 图层管理按钮事件
    document
      .getElementById('layerManagementBtn')
      .addEventListener('click', () => this.toggleLayerManagement())
    ...
    document
      .getElementById('setRouteBtn')
      .addEventListener('click', () => this.setNavigationMode())
    document
      .getElementById('clearNavigationBtn')
      .addEventListener('click', () => this.clearNavigation())

    // 空间分析事件
    document
      .getElementById('spatialAnalysisBtn')
      .addEventListener('click', () => this.toggleAnalysisPanel())
    document
      .getElementById('viewshedBtn')
      .addEventListener('click', () => this.togglePanel('viewshedPanel'))
    document
      .getElementById('sightlineBtn')
      .addEventListener('click', () => this.togglePanel('sightlinePanel'))
    document
      .getElementById('shadowAnalysisBtn')
      .addEventListener('click', () => this.togglePanel('shadowPanel'))
    document
      .getElementById('skylineAnalysisBtn')
      .addEventListener('click', () => this.togglePanel('skylinePanel'))

    document
      .getElementById('clearAnalysisBtn')
      .addEventListener('click', () => this.clearAllAnalysis())

    // 清除按钮已移至各自的弹窗面板中

    // 场景效果事件
    document
      .getElementById('lightingBtn')
      .addEventListener('click', () => this.adjustLighting())

    document
      .getElementById('bloomBtn')
      .addEventListener('click', () => this.toggleBloom())

    // 测量分析按钮事件
    document
      .getElementById('distanceMeasureBtn')
      .addEventListener('click', () => this.performDistanceMeasure())

    document
      .getElementById('areaMeasureBtn')
      .addEventListener('click', () => this.performAreaMeasure())

    document
      .getElementById('heightMeasureBtn')
      .addEventListener('click', () => this.performHeightMeasure())

    document
      .getElementById('clearMeasureBtn')
      .addEventListener('click', () => this.clearMeasurements())

    // 面板关闭事件（用于弹出面板）
    document
      .getElementById('closeNavigation')
      .addEventListener('click', () => this.hidePanel('navigationPanel'))
    document
      .getElementById('closeAnalysis')
      .addEventListener('click', () => this.hidePanel('analysisPanel'))
    document
      .getElementById('closeInfo')
      .addEventListener('click', () => this.hidePanel('infoPanel'))
    document
      .getElementById('closeViewshed')
      .addEventListener('click', () => this.hidePanel('viewshedPanel'))
    document
      .getElementById('closeSightline')
      .addEventListener('click', () => this.hidePanel('sightlinePanel'))
    document
      .getElementById('closeShadowPanel')
      .addEventListener('click', () => this.hidePanel('shadowPanel'))
    ...
    document
      .getElementById('startWalk')
      .addEventListener('click', () => this.startWalkthrough())
    document
      .getElementById('stopWalk')
      .addEventListener('click', () => this.stopWalkthrough())
    document
      .getElementById('clearNavigation')
      .addEventListener('click', () => this.clearNavigation())
    document
      .getElementById('floorSelect')
      .addEventListener('change', (e) => this.changeFloor(e.target.value))

    // 可视域分析面板事件
    document
      .getElementById('drawViewshed')
      .addEventListener('click', () => this.drawViewshed())
    document
      .getElementById('drawClipPlane')
      .addEventListener('click', () => this.drawClipPlane())
    document
      .getElementById('clearViewshed')
      .addEventListener('click', () => this.clearSpecificAnalysis('viewshed'))
    document
      .getElementById('clipMode')
      .addEventListener('change', (e) => this.setClipMode(e.target.value))
    document
      .getElementById('propertyEditor')
      .addEventListener('click', () => this.showPropertyEditor())

    // 通视分析面板事件
    document
      .getElementById('addViewPoint')
      .addEventListener('click', () => this.addSightlineViewPoint())
    document
      .getElementById('addTargetPoint')
      .addEventListener('click', () => this.addSightlineTargetPoint())
    document
      .getElementById('clearSightline')
      .addEventListener('click', () => this.clearSpecificAnalysis('sightline'))

    // 阴影分析面板事件
    document
      .getElementById('startShadowAnalysis')
      .addEventListener('click', () => this.startShadowAnalysis())
    document
      .getElementById('shadowSunlight')
      .addEventListener('click', () => this.performSunlightEffect())
    document
      .getElementById('getShadowRatio')
      .addEventListener('click', () => this.getShadowRatio())
    document
      .getElementById('clearShadow')
      .addEventListener('click', () => this.clearSpecificAnalysis('shadow'))

    // 天际线分析面板事件
    document
      .getElementById('closeSkylinePanel')
      .addEventListener('click', () => this.hidePanel('skylinePanel'))
    document
      .getElementById('extractSkyline')
      .addEventListener('click', () => this.extractSkyline())
    document
      .getElementById('getSkyline2D')
      .addEventListener('click', () => this.getSkyline2D())
    document
      .getElementById('clearSkyline')
      .addEventListener('click', () => this.clearSkylineAnalysis())

    // 属性编辑面板事件
    document
      .getElementById('closeProperty')
      .addEventListener('click', () => this.hidePanel('propertyPanel'))
    this.bindPropertyControls()

    // 空间分析主面板事件 - 打开子面板
    document
      .getElementById('openClipPanel')
      .addEventListener('click', () => this.togglePanel('clipPanel'))

    // 空间分析子面板关闭事件
    document
      .getElementById('closeClip')
      .addEventListener('click', () => this.hidePanel('clipPanel'))

    // 空间分析功能按钮事件
    document
      .getElementById('startClip')
      .addEventListener('click', () => this.startClipAnalysis())
    document
      .getElementById('clearClip')
      .addEventListener('click', () => this.clearSpecificAnalysis('clip'))

    // 主界面剖面图表关闭按钮事件
    document
      .getElementById('closeMainProfileChart')
    ...
    // 交互取消按钮事件
    document
      .getElementById('cancelInteraction')
      .addEventListener('click', () => this.cancelInteraction())
  ...
  bindMouseEvents() {
    ...
    if (!handler) {
      ...
      if (
        ...
        // 最后尝试创建一个新的handler
        try {
          handler = new SuperMap3D.ScreenSpaceEventHandler(
  ...
  testMouseEventBinding() {
    try {
      ...
      if (
        handler &&
        handler._inputEvents &&
        handler._inputEvents[SuperMap3D.ScreenSpaceEventType.LEFT_CLICK]
      ) {
  ...
  async loadModelFromUrl(url, modelName) {
    try {
      ...
      switch (fileExtension.toLowerCase()) {
        case 's3m':
          await this.loadS3MFile(url, modelName)
          break
        case 'gltf':
        case 'glb':
          await this.loadGLTFFile(url, modelName)
          break
        case 'obj':
          await this.loadOBJFile(url, modelName)
          break
        case '3ds':
        case 'dae':
        case 'fbx':
        ...
        case 'x3d':
        case 'wrl':
          await this.loadGenericFile(url, modelName, fileExtension)
          break
        default:
  ...
  showSimpleBatchLoadDialog() {
    ...
    dialog.innerHTML = `
            <div style="text-align: center; margin-bottom: 25px;">
                <div style="width: 70px; height: 70px; margin: 0 auto 18px; background: linear-gradient(135deg, #3b82f6, #06b6d4); border-radius: 50%; display: flex; align-items: center; justify-content: center; box-shadow: 0 8px 25px rgba(59, 130, 246, 0.4);">
                    <span style="font-size: 32px;">📁</span>
                </div>
                <h3 style="margin: 0 0 8px; font-size: 22px; font-weight: 600; color: #1f2937;">批量加载模型</h3>
                <p style="margin: 0; color: #6b7280; font-size: 14px;">批量加载功能正在运行中...</p>
            </div>
            <div style="margin: 25px 0;">
                <button onclick="window.selectMultipleFiles()" style="
                    width: 100%;
                    margin: 12px 0;
                    padding: 18px 20px;
                ...
                ">
                    ...
                    <div style="flex: 1;">选择多个文件</div>
                </button>
                <button onclick="window.selectFolder()" style="
                    width: 100%;
                    margin: 12px 0;
                    padding: 18px 20px;
                ...
                ">
                    ...
                    <div style="flex: 1;">选择整个文件夹</div>
                </button>
            </div>
            <div style="text-align: center; margin-top: 25px;">
                <button onclick="window.closeSimpleDialog()" style="
                    padding: 12px 35px;
                    background: #f3f4f6;
                    color: #374151;
    ...
    return new Promise((resolve) => {
      ...
      dialog.innerHTML = `
                <div style="text-align: center; margin-bottom: 25px;">
                    ...
                    <h3 style="margin: 0 0 8px; font-size: 22px; font-weight: 600; color: #1f2937;">批量加载模型</h3>
                    <p style="margin: 0; color: #6b7280; font-size: 14px;">选择批量加载方式，一次性加载多个模型文件</p>
                </div>
                <div style="margin: 25px 0;">
                    <button class="batch-load-option" data-option="multiple" style="
                        width: 100%;
                        margin: 12px 0;
                        padding: 18px 20px;
                        background: linear-gradient(135deg, #22c55e, #16a34a);
                    ...
                    ">
                        ...
                        <div style="flex: 1;">
                            <div>选择多个文件</div>
                            <small style="display: block; color: rgba(255,255,255,0.8); font-size: 12px; font-weight: 400; margin-top: 3px;">按住 Ctrl 键选择多个模型文件</small>
                        </div>
                    </button>
                    <button class="batch-load-option" data-option="folder" style="
                        width: 100%;
                        margin: 12px 0;
                        padding: 18px 20px;
                        background: linear-gradient(135deg, #06b6d4, #0891b2);
                    ...
                    ">
                        ...
                        <div style="flex: 1;">
                            <div>选择整个文件夹</div>
                            <small style="display: block; color: rgba(255,255,255,0.8); font-size: 12px; font-weight: 400; margin-top: 3px;">自动加载文件夹中的所有模型文件</small>
                        </div>
                    </button>
                </div>
                <div style="background: #f3f4f6; padding: 15px; border-radius: 10px; margin: 20px 0; border: 1px solid #e5e7eb;">
                    <div style="font-size: 12px; color: #6b7280; text-align: center;">
                        <strong style="color: #374151;">支持格式:</strong> S3M, GLTF, GLB, OBJ, 3DS, DAE, FBX, PLY, STL, X3D, WRL
                    </div>
                </div>
                <div style="text-align: center; margin-top: 25px;">
                    <button id="cancelBatchLoad" style="
                        padding: 12px 35px;
                        background: #f3f4f6;
                        color: #374151;
                        border: 1px solid #d1d5db;
      ...
      document
        .getElementById('cancelBatchLoad')
        .addEventListener('click', () => {
          document.body.removeChild(dialog)
          resolve(null)
        })
  ...
  showModelLoadOptionDialog() {
    return new Promise((resolve) => {
      ...
      document
        .getElementById('cancelLoadOption')
        .addEventListener('click', () => {
          document.body.removeChild(dialog)
          resolve(null)
        })
  ...
  async loadSingleFile() {
    return new Promise((resolve, reject) => {
      ...
      fileInput.type = 'file'
      fileInput.accept =
        '.s3m,.gltf,.glb,.obj,.3ds,.dae,.fbx,.ply,.stl,.x3d,.wrl'
  ...
  async loadMultipleFileModels(files) {
    ...
    if (totalFiles > 1000) {
      const confirmed = confirm(
        `您要加载 ${totalFiles} 个文件，这可能会消耗大量内存并导致性能问题。\n\n建议：\n- 分批加载（每次不超过500个文件）\n- 确保有足够的内存\n- 关闭其他占用内存的程序\n\n是否继续？`,
      )
      if (!confirmed) {
  ...
  async loadSingleFileInBatch(file, index, totalFiles = null) {
    ...
    console.log(
      `批量加载文件 ${index + 1}: ${file.name}, 格式: ${fileExtension}`,
    )
    ...
    switch (fileExtension) {
      ...
      case 'gltf':
      case 'glb':
        return await this.loadGLTFFileInBatch(
          fileUrl,
          file.name,
          offsetX,
          offsetY,
      ...
      case 'obj':
        return await this.loadOBJFileInBatch(
          fileUrl,
          file.name,
          offsetX,
          offsetY,
      ...
      case 'x3d':
      case 'wrl':
        return await this.loadGenericFileInBatch(
          fileUrl,
          file.name,
          fileExtension,
          offsetX,
  ...
  async loadS3MFile(fileUrl, fileName) {
    try {
      ...
      if (
        fileName.toLowerCase().endsWith('.scp') ||
  ...
  async loadGenericFile(fileUrl, fileName, fileExtension) {
    try {
      ...
      throw new Error(
        `${fileExtension.toUpperCase()}文件加载失败: ${error.message}`,
      )
    }
  }
      ...
      if (
        ...
        SuperMap3D.when(
          ...
          (error) => {
            ...
            this.showErrorDialog(
              '加载失败',
              '本地模型文件不存在或格式错误。请检查文件路径：' + config.url,
            )
          },
        )
      } else {
  ...
  toggleLeftPanel() {
    ...
    const expandBtn = document.getElementById('expandLeftPanel')
  ...
  removeSelectedLayer() {
    ...
    // 确认删除
    const layerNames = Array.from(selectedItems)
      .map((item) => {
    ...
    indices.forEach((layerIndex) => {
      if (layerIndex >= 0 && layerIndex < this.layers.length) {
        ...
        if (this.indoorNavigation) {
          console.log(
            `🧹 移除模型 "${layerInfo.name}"：清除相关的起点、终点和路径`,
          )
    ...
    try {
      ...
      if (layerInfo.entity && layerInfo.entity.position) {
        try {
          ...
          if (positionValue) {
            const cartographic =
              SuperMap3D.Cartographic.fromCartesian(positionValue)
      ...
      if (
        layerInfo.entity &&
        layerInfo.entity.model &&
        layerInfo.entity.model.scale
      ) {
        try {
          let scaleValue
  ...
  updateLayerPanel() {
    ...
    if (!this.layers || this.layers.length === 0) {
      ...
      layerTree.innerHTML =
        '<div style="padding: 20px; text-align: center; color: #999;">暂无图层</div>'
      return
    }
    ...
    this.layers.forEach((layerInfo, index) => {
      ...
      layerItem.addEventListener('click', (e) => {
        if (
          ...
          !e.target.classList.contains('layer-checkbox')
        ) {
          // 切换选中状态
  ...
  updateLayerStats() {
    ...
    const visibleCount = this.layers
      ? this.layers.filter((layer) => layer.visible !== false).length
      : 0
    ...
    this.handler.setInputAction((event) => {
      ...
      if (position) {
        if (!startPoint) {
          ...
          this.digitalTwinAnalysis.performSightlineAnalysis(
            startPoint,
            position,
          )
  ...
  pickModelPosition(windowPosition) {
    try {
      ...
      // 如果没有拾取到模型，尝试拾取地形
      const terrainPosition = this.viewer.camera.pickEllipsoid(
        windowPosition,
        this.scene.globe.ellipsoid,
      )
      if (terrainPosition) {
  ...
  performProfileAnalysis() {
    // 使用交互式分析管理器处理剖面分析
    if (
      this.digitalTwinAnalysis &&
      this.digitalTwinAnalysis.interactiveManager
    ) {
  ...
  clearAllAnalysis() {
    try {
      ...
      console.log('✅ 所有分析结果和导航路径已清除')
      this.updateStatus('所有分析结果和导航路径已清除')
      this.showSuccessMessage('所有分析结果和导航路径已清除')
    } catch (error) {
      console.error('❌ 清除所有分析结果失败:', error)
      this.updateStatus('清除分析结果失败')
      this.showErrorDialog(
        '清除失败',
        '清除分析结果时发生错误：' + error.message,
      )
    }
  }
   ...
   * 执行距离测量
   */
  performDistanceMeasure() {
    try {
      console.log('📏 开始距离测量...')
      this.updateStatus('正在执行距离测量...')

      if (this.digitalTwinAnalysis) {
        this.digitalTwinAnalysis.performDistanceMeasure(0)
        this.showSuccessMessage('距离测量已启动，请在场景中点击测量起点和终点')
      } else {
        console.error('❌ 数字孪生分析模块未初始化')
        this.showErrorDialog('测量失败', '分析模块未初始化')
      }
    } catch (error) {
      console.error('❌ 距离测量失败:', error)
      this.showErrorDialog('测量失败', error.message)
    }
  }
  ...
  performAreaMeasure() {
    try {
      console.log('📐 开始面积测量...')
      this.updateStatus('正在执行面积测量...')

      if (this.digitalTwinAnalysis) {
        this.digitalTwinAnalysis.performAreaMeasure(0)
  ...
  performHeightMeasure() {
    try {
      console.log('📊 开始高度测量...')
      this.updateStatus('正在执行高度测量...')

      if (this.digitalTwinAnalysis) {
        this.digitalTwinAnalysis.performHeightMeasure()
        this.showSuccessMessage('高度测量已启动，请在场景中点击测量起点和终点')
      } else {
        console.error('❌ 数字孪生分析模块未初始化')
        this.showErrorDialog('测量失败', '分析模块未初始化')
      }
  ...
  clearMeasurements(measureType = 'all') {
    try {
      ...
      this.updateStatus('正在清除测量结果...')

      if (this.digitalTwinAnalysis) {
        this.digitalTwinAnalysis.clearMeasurements(measureType)
        this.showSuccessMessage('测量结果已清除')
      } else {
        console.error('❌ 数字孪生分析模块未初始化')
        this.showErrorDialog('清除失败', '分析模块未初始化')
      }
    } catch (error) {
      console.error('❌ 清除测量失败:', error)
      this.showErrorDialog('清除失败', error.message)
    }
  }
  ...
  clearSpecificAnalysis(analysisType) {
    try {
      ...
      this.showErrorDialog(
        '清除失败',
        `清除${analysisType}分析结果时发生错误：` + error.message,
      )
    }
  }
  ...
  startShadowAnalysis() {
    try {
      ...
      // 获取分析参数
      const shadowDate =
        document.getElementById('shadowDate')?.value ||
        new Date().toISOString().split('T')[0]
      const startTime =
        document.getElementById('shadowStartTime')?.value || '10'
      const endTime = document.getElementById('shadowEndTime')?.value || '14'
      const bottomHeight =
        document.getElementById('shadowBottomHeight')?.value || '20'
      const extrudeHeight =
        document.getElementById('shadowExtrudeHeight')?.value || '20'
      ...
      if (this.digitalTwinAnalysis) {
        this.digitalTwinAnalysis.getShadowRatio((result) => {
          if (result) {
            ...
            this.updateStatus(
              `阴影率: ${(result.shadowRatio * 100).toFixed(2)}%`,
            )
            this.showSuccessMessage(
              `阴影率: ${(result.shadowRatio * 100).toFixed(2)}%`,
            )
  ...
  extractSkyline() {
    try {
      ...
      if (success) {
        ...
        this.showSuccessMessage('天际线提取完成，可以查看二维天际线')
      } else {
        this.showErrorDialog(
          '提取失败',
          '天际线提取失败，请检查场景是否支持深度纹理',
        )
      }
    } catch (error) {
  ...
  async getSkyline2D() {
    try {
      ...
      this.showSuccessMessage(
        `二维天际线图表已生成，包含 ${skylineData.x.length} 个数据点，可通过鼠标交互查看详细数据`,
      )
    } catch (error) {
      console.error('❌ 获取二维天际线失败:', error)
      this.showErrorDialog(
        '获取失败',
        '获取二维天际线时发生错误：' + error.message,
      )
    }
  }
  ...
  // 注：setLimitBody和getSkylineArea功能已移除，以简化天际线分析清除逻辑
  ...
  clearSkylineAnalysis() {
    try {
      ...
      // 清除可能存在的其他图表容器（只清除图表相关的容器，不隐藏界面元素）
      const allChartContainers = document.querySelectorAll(
        '[id*="skyline"], [class*="skyline"]',
      )
      allChartContainers.forEach((container) => {
        // 只处理图表容器，不隐藏界面面板和按钮
        if (
          container.id !== 'skylineChart' &&
      ...
      this.showSuccessMessage(
        '天际线分析已完全清除，包括所有图表实例、事件监听器和相关数据',
      )
  ...
  showLoadingProgress(message) {
    const loadingDiv =
      document.getElementById('loadingbar') || document.createElement('div')
  ...
  adjustLighting() {
    ...
    if (lightSource.ambientLightColor.red > 0.5) {
      const nightColor = EFFECT_CONFIG.LIGHTING.AMBIENT_LIGHT.NIGHT
      lightSource.ambientLightColor = new SuperMap3D.Color(
        nightColor[0], // red
        nightColor[1], // green
        nightColor[2], // blue
        nightColor[3], // alpha
      ...
      const dayColor = EFFECT_CONFIG.LIGHTING.AMBIENT_LIGHT.DAY
      lightSource.ambientLightColor = new SuperMap3D.Color(
        dayColor[0], // red
        dayColor[1], // green
        dayColor[2], // blue
    ...
    try {
      ...
      if (this.navigationMode) {
        ...
        if (cartesian) {
          ...
          this.updateStatus(
            `已添加${this.routePoints.length}个路径点，继续点击添加更多点，右键点击结束路径选择`,
          )
        } else {
  ...
  /**
   * 鼠标右键点击事件 - 结束路径选择并计算路径
   */
  onRightClick(event) {
    try {
      console.log('🖱️ 鼠标右键点击事件触发')

      if (this.navigationMode && this.routePoints.length >= 2) {
        console.log('🧭 结束路径选择，开始计算路径')
        ...
        this.updateStatus(
          `路径设置完成，共${this.routePoints.length}个路径点，正在计算路径...`,
        )
      ...
      } else if (this.navigationMode && this.routePoints.length < 2) {
        console.warn('⚠️ 路径点数量不足，至少需要2个点')
        this.updateStatus('路径点数量不足，至少需要设置2个路径点')
      }
    } catch (error) {
      console.error('❌ 鼠标右键点击事件处理失败:', error)
    }
  }
  ...
  updateRoutePointsDisplay() {
    ...
    if (startPointInput) {
      if (this.routePoints.length >= 1) {
        startPointInput.value = `起点已设置 (第1个点)`
      } else {
        startPointInput.value = ''
      }
    }
    ...
    if (endPointInput) {
      if (this.routePoints.length >= 2) {
        endPointInput.value = `终点已设置 (第${this.routePoints.length}个点)`
      } else {
        endPointInput.value = ''
      }
    }
    ...
    // 显示路径点数量
    const routePointsInfo = document.getElementById('routePointsInfo')
    if (routePointsInfo) {
      routePointsInfo.textContent = `路径点数量: ${this.routePoints.length}`
    }
  }
  ...
  onMouseMove(event) {
    ...
    if (cartesian) {
      ...
      const latitude = SuperMap3D.Math.toDegrees(cartographic.latitude).toFixed(
        6,
      )
      const height = cartographic.height.toFixed(2)
  ...
  showBatchLoadingResult(loadedFiles, failedFiles, totalFiles) {
    ...
    const successColor = loadedFiles > 0 ? '#22c55e' : '#6b7280'
    const failColor = failedFiles.length > 0 ? '#ef4444' : '#6b7280'

    let resultHtml = `
            <div style="text-align: center; margin-bottom: 20px;">
                <div style="width: 60px; height: 60px; margin: 0 auto 15px; background: linear-gradient(135deg, #3b82f6, #06b6d4); border-radius: 50%; display: flex; align-items: center; justify-content: center;">
                    <span style="font-size: 28px; color: white;">📁</span>
                </div>
                <h3 style="margin: 0 0 5px; font-size: 20px; font-weight: 600; color: #1f2937;">批量加载完成</h3>
                <p style="margin: 0; color: #6b7280; font-size: 13px;">Batch Loading Complete</p>
            </div>
            <div style="display: flex; gap: 15px; margin: 20px 0; justify-content: center;">
                <div style="flex: 1; text-align: center; padding: 15px; background: #f3f4f6; border-radius: 12px;">
                    <div style="font-size: 28px; font-weight: 700; color: #374151;">${totalFiles}</div>
                    <div style="font-size: 12px; color: #6b7280; margin-top: 5px;">总文件数</div>
                </div>
                <div style="flex: 1; text-align: center; padding: 15px; background: rgba(34, 197, 94, 0.1); border-radius: 12px; border: 1px solid rgba(34, 197, 94, 0.3);">
                    <div style="font-size: 28px; font-weight: 700; color: ${successColor};">${loadedFiles}</div>
                    <div style="font-size: 12px; color: #16a34a; margin-top: 5px;">成功加载</div>
                </div>
                <div style="flex: 1; text-align: center; padding: 15px; background: rgba(239, 68, 68, 0.1); border-radius: 12px; border: 1px solid rgba(239, 68, 68, 0.3);">
                    <div style="font-size: 28px; font-weight: 700; color: ${failColor};">${failedFiles.length}</div>
                    <div style="font-size: 12px; color: #dc2626; margin-top: 5px;">加载失败</div>
                </div>
            </div>
        `
    ...
    if (failedFiles.length > 0) {
      resultHtml += `
                <div style="margin-top: 20px;">
                    <h4 style="margin: 0 0 10px; font-size: 14px; color: #374151; font-weight: 600;">失败文件列表:</h4>
                    <div style="max-height: 120px; overflow-y: auto; background: #fef2f2; padding: 10px; border-radius: 8px; border: 1px solid rgba(239, 68, 68, 0.2);">
                        ${failedFiles
                          .map(
                            (file) => `
                            <div style="margin: 8px 0; padding: 8px; background: white; border-radius: 6px; font-size: 12px; border-left: 3px solid #ef4444;">
                                <div style="font-weight: 600; color: #374151;">${file.name}</div>
                                <div style="color: #dc2626; margin-top: 3px;">${file.error}</div>
                            </div>
                        `,
                          )
                          .join('')}
    ...
    const btn = dialog.querySelector('.close-result-btn')
    btn.addEventListener('mouseenter', () => {
      ...
      return new Promise((resolve, reject) => {
        try {
          ...
          SuperMap3D.when(
            ...
            (layer) => {
              console.log(
                `✅ S3M文件作为3D Tiles图层加载成功: ${fileName}`,
                layer,
              )
            ...
            (error) => {
              console.error(
                `❌ S3M文件作为3D Tiles图层加载失败: ${fileName}`,
                error,
              )
  ...
  async loadS3MAsPrimitive(fileUrl, fileName, index) {
    try {
      ...
      throw new Error(
        `S3M文件无法加载，可能文件格式不正确或需要配置文件: ${error.message}`,
      )
    }
  }
  ...
  async loadGenericFileInBatch(
    ...
    try {
      ...
      console.log(
        `批量${fileExtension.toUpperCase()}文件加载成功: ${fileName}`,
        entity,
      )
    ...
    try {
      ...
      if (this.viewer.entities.values.length > 0) {
        ...
        entities.forEach((entity) => {
          if (entity.position) {
            const cartographic = SuperMap3D.Cartographic.fromCartesian(
              entity.position.getValue(SuperMap3D.JulianDate.now()),
            )
        ...
        if (validCount > 0) {
          ...
          console.log(
            `✅ 视角重置到模型中心: (${centerX.toFixed(4)}, ${centerY.toFixed(4)}, ${centerZ.toFixed(1)})`,
          )
        } else {
          this.setDefaultView()
        }
  ...
  checkMemoryUsage() {
    try {
      ...
      if (performance.memory) {
        ...
        console.log(
          `💾 内存使用: ${(memInfo.usedJSHeapSize / 1024 / 1024).toFixed(1)}MB / ${(memInfo.jsHeapSizeLimit / 1024 / 1024).toFixed(1)}MB (${usedPercent.toFixed(1)}%)`,
        )
    ...
    try {
      ...
      if (canvas) {
        ...
        if (canvas.width <= 0 || canvas.height <= 0) {
          console.warn(
            `⚠️ Canvas尺寸可能无效: ${canvas.width}x${canvas.height}，但允许继续`,
          )
        }
      ...
      try {
        if (
          ...
          this.viewer.scene.context.isDestroyed &&
          this.viewer.scene.context.isDestroyed()
        ) {
  ...
  conservativeFlyToModels() {
    try {
      ...
      if (validCount > 0) {
        ...
        console.log(
          `✅ 保守飞行完成，视角设置到: (${centerX.toFixed(4)}, ${centerY.toFixed(4)}, ${centerZ.toFixed(1)})`,
        )
        this.enableCameraControls()
        // 重新绑定鼠标事件，确保导航功能正常
        this.rebindMouseEventsAfterModelLoad()
  ...
  setupWebGLContextHandling() {
    try {
      if (this.viewer && this.viewer.canvas) {
        ...
        console.log('[Main] WebGL上下文事件处理已设置')
      }
    } catch (error) {
...
function bindViewshedPropertyControls() {
  ...
  const horizontalFovValue = document.getElementById(
    'modal-horizontal-fov-value',
  )
...
function updateViewshedProperty(propertyName, value) {
  if (
    ...
    window.realityTwin3DAnalysisTool.digitalTwinAnalysis.updateViewshedProperty(
      propertyName,
      value,
    )
...
function selectClipMode(mode) {
  ...
  if (
    ...
    window.realityTwin3DAnalysisTool.digitalTwinAnalysis.setViewshedClipMode(
      mode,
    )
...
function applyViewshedProperties() {
  ...
  const properties = {
    ...
    horizontalFov: parseFloat(
      document.getElementById('modal-horizontal-fov').value,
    ),
    verticalFov: parseFloat(
      document.getElementById('modal-vertical-fov').value,
    ),
 ...
 * @param {string} suggestedName - 建议的模型名称
 */
RealityTwin3DAnalysisTool.prototype.loadSelectedFile = async function (
  file,
  suggestedName = null,
) {
  try {
    console.log('📁 加载选中的文件:', file.name, '建议名称:', suggestedName)
    ...
    switch (fileExtension) {
      case 's3m':
        result = await this.loadS3MFile(fileUrl, suggestedName || file.name)
        break
      case 'gltf':
      case 'glb':
        result = await this.loadGLTFFile(fileUrl, suggestedName || file.name)
        break
      case 'obj':
        result = await this.loadOBJFile(fileUrl, suggestedName || file.name)
        break
      case '3ds':
      case 'dae':
      case 'fbx':
      ...
      case 'x3d':
      case 'wrl':
        result = await this.loadGenericFile(
          fileUrl,
