...
class DigitalTwinAnalysis {
    ...
    handleRenderError(error) {
        try {
            ...
            if (this.errorCount > 3) {
                ...
                // 清除临时实体
                if (this.interactiveManager && typeof this.interactiveManager.clearTempEntities === 'function') {
    ...
    handleViewshedRenderError() {
        try {
            console.log('🔧 处理可视域分析渲染错误...');
            
            // 特别清除可视域分析相关对象
            const viewshedResult = this.analysisResults.get('viewshed');
            if (viewshedResult) {
                // 销毁ViewShed3D对象
                if (viewshedResult.viewshed3D && typeof viewshedResult.viewshed3D.destroy === 'function') {
                    try {
                        viewshedResult.viewshed3D.destroy();
                        console.log('✅ 销毁ViewShed3D对象成功');
                    } catch (destroyError) {
     ...
     */
    performViewshedAnalysis(viewPoint, distance = 1000, pitch = -30, heading = 0, options = {}) {
        try {
            console.log('🔍 开始可视域分析...', { viewPoint, distance, pitch, heading });
            ...
            if (!this.scene.pickPositionSupported) {
                ...
                return this.performCustomViewshedAnalysis(viewPoint, distance, pitch, heading, options);
            }
            ...
            console.log('🔧 使用SuperMap3D.ViewShed3D官方实现');
            return this.performOfficialViewshedAnalysis(viewPoint, distance, pitch, heading, options);
     ...
     * @param {Object} options 可选参数
     */
    performOfficialViewshedAnalysis(viewPoint, distance = 1000, pitch = -30, heading = 0, options = {}) {
        try {
            console.log('🔍 使用SuperMap3D.ViewShed3D官方实现...', { viewPoint, distance, pitch, heading });
            ...
            if (!viewPoint || !SuperMap3D.defined(viewPoint) || 
                ...
                isNaN(viewPoint.x) || isNaN(viewPoint.y) || isNaN(viewPoint.z)) {
            ...
            if (viewPoint.longitude !== undefined && viewPoint.latitude !== undefined && viewPoint.height !== undefined) {
                // 如果viewPoint已经是经纬度格式，直接使用
                longitude = viewPoint.longitude;
                latitude = viewPoint.latitude;
                height = viewPoint.height + 1.8; // 添加人眼高度
                console.log('使用经纬度格式的观察点:', { longitude, latitude, height });
            } else {
                ...
                console.log('从Cartesian3转换的经纬度坐标:', { longitude, latitude, height });
            }
            ...
            // 设置可视域分析参数
            console.log('设置可视域分析参数:', { longitude, latitude, height, heading, pitch, distance });
            ...
            try {
                ...
                if (options.visibleColor) {
                    if (typeof options.visibleColor === 'string') {
                        ...
                        console.log('转换可见区域颜色:', options.visibleColor, '->', visibleColor);
                ...
                if (options.hiddenColor) {
                    if (typeof options.hiddenColor === 'string') {
                        ...
                        console.log('转换不可见区域颜色:', options.hiddenColor, '->', hiddenColor);
                ...
                // 确保颜色对象有效且数值在正确范围内
                if (!visibleColor || typeof visibleColor.red !== 'number' || 
                    isNaN(visibleColor.red) || isNaN(visibleColor.green) || isNaN(visibleColor.blue) || isNaN(visibleColor.alpha) ||
                    visibleColor.red < 0 || visibleColor.red > 1 || 
                    visibleColor.green < 0 || visibleColor.green > 1 || 
                    visibleColor.blue < 0 || visibleColor.blue > 1 || 
                    visibleColor.alpha < 0 || visibleColor.alpha > 1) {
                    console.warn('⚠️ 可见区域颜色无效，使用默认绿色');
                    visibleColor = new SuperMap3D.Color(0.0, 1.0, 0.0, 0.5);
                }
                
                if (!hiddenColor || typeof hiddenColor.red !== 'number' || 
                    isNaN(hiddenColor.red) || isNaN(hiddenColor.green) || isNaN(hiddenColor.blue) || isNaN(hiddenColor.alpha) ||
                    hiddenColor.red < 0 || hiddenColor.red > 1 || 
                    hiddenColor.green < 0 || hiddenColor.green > 1 || 
                    hiddenColor.blue < 0 || hiddenColor.blue > 1 || 
                    hiddenColor.alpha < 0 || hiddenColor.alpha > 1) {
            ...
            try {
                ...
                if (this.scene.primitives && this.scene.primitives.length > 0) {
                    ...
                    for (let i = 0; i < this.scene.primitives.length; i++) {
                        ...
                        if (primitive && (primitive.constructor.name.includes('ViewShed') || 
                            primitive.constructor.name.includes('Viewshed') ||
                            primitive._viewshed || primitive.viewshed)) {
                            isAddedToScene = true;
                            sceneInfo.push(`primitives中发现可视域对象: ${primitive.constructor.name}`);
                            break;
                        }
                    }
                }
                ...
                // 检查viewshed3D对象本身的状态
                if (viewshed3D && (viewshed3D._built || viewshed3D.built || viewshed3D.isBuilt)) {
                    isAddedToScene = true;
                            ...
                            try {
                                ...
                                const isValidVec4 = (vec) => {
                                    return Array.isArray(vec) && vec.length === 4 && 
                                           vec.every(v => typeof v === 'number' && !isNaN(v) && v >= 0 && v <= 1);
                                };
                                ...
                                if (isValidVec4(visibleVec4) && isValidVec4(hiddenVec4)) {
                                    ...
                                    try {
                                        // 方式1：直接设置Color对象
                                        viewshed3D.visibleAreaColor = this._pendingColors.visibleColor;
                                        ...
                                        try {
                                            ...
                                            try {
                                                ...
                                                viewshed3D.visibleAreaColor = new SuperMap3D.Cartesian4(...visibleVec4);
                                                viewshed3D.hiddenAreaColor = new SuperMap3D.Cartesian4(...hiddenVec4);
            ...
            if (typeof window !== 'undefined' && window.showMessage) {
                window.showMessage('可视域分析完成', '使用SuperMap3D官方实现，显示精确可视区域', 'success');
            }
     ...
     * @param {Object} options 可选参数
     */
    performCustomViewshedAnalysis(viewPoint, distance = 1000, pitch = -30, heading = 0, options = {}) {
        try {
            console.log('🔍 使用自定义可视域分析...', { viewPoint, distance, pitch, heading });
            ...
            // 创建可视域扇形几何体
            const viewshedGeometry = this.createViewshedGeometry(viewPoint, distance, heading, horizontalAngle, pitch, verticalAngle);
            ...
            const viewshedEntity = this.viewer.entities.add({
                ...
                polygon: {
                    ...
                    height: height,
                    extrudedHeight: height + (distance * Math.sin(SuperMap3D.Math.toRadians(Math.abs(pitch)))),
                    closeTop: false,
                    closeBottom: false
                }
            });
            ...
            // 添加方向指示线
            const directionEndPoint = this.calculateDirectionPoint(viewPoint, distance, heading);
            ...
            this.analysisEntities.push(viewshedEntity, viewPointEntity, directionEntity);
            ...
            // 显示提示信息
            if (typeof window !== 'undefined' && window.showMessage) {
                window.showMessage('可视域分析完成', '使用自定义实现，显示近似可视区域', 'info');
            }
            ...
            // 显示用户友好的错误信息
            if (typeof window !== 'undefined' && window.showErrorMessage) {
                window.showErrorMessage('可视域分析失败', '无法执行可视域分析: ' + error.message);
            }
     ...
     * 创建可视域几何体
     */
    createViewshedGeometry(viewPoint, distance, heading, horizontalAngle, pitch, verticalAngle) {
        try {
            console.log('创建可视域几何体:', { distance, heading, horizontalAngle, pitch, verticalAngle });
            ...
            for (let i = 0; i <= steps; i++) {
                const angle = heading - halfHorizontalAngle + (horizontalAngle * i / steps);
                const endPoint = this.calculateDirectionPoint(viewPoint, distance, angle);
    ...
    calculateDirectionPoint(startPoint, distance, heading) {
        ...
        const deltaLon = (distance * Math.sin(headingRadians)) / (111320 * Math.cos(SuperMap3D.Math.toRadians(latitude))); // 经度变化
    ...
    performSightlineAnalysis(startPoint, endPoint, options = {}) {
        try {
            ...
            if (distance > 10000) {
                console.warn('⚠️ 两点距离较远(' + distance.toFixed(2) + 'm)，分析可能不准确');
            }
            ...
            if (!this.sightlineAnalysis) {
                try {
                    console.log('🔧 创建SuperMap3D.Sightline对象...');
                    ...
                    if (this.sightlineAnalysis.visibleColor !== undefined) {
                        this.sightlineAnalysis.visibleColor = new SuperMap3D.Color(0, 1, 0, 1); // 绿色表示可见
                    }
                    if (this.sightlineAnalysis.hiddenColor !== undefined) {
                        this.sightlineAnalysis.hiddenColor = new SuperMap3D.Color(1, 0, 0, 1);  // 红色表示不可见
                    }
            ...
            // 转换坐标为经纬度并验证
            const startCartographic = SuperMap3D.Cartographic.fromCartesian(startPoint);
            ...
            // 创建观察点标记
            const observerMarker = this.viewer.entities.add({
                id: `sightline-observer-${Date.now()}`,
                position: startPoint,
                point: {
                    pixelSize: 12,
                    color: SuperMap3D.Color.YELLOW,
            ...
            // 创建目标点标记
            const targetMarker = this.viewer.entities.add({
                id: `sightline-target-${Date.now()}`,
                position: endPoint,
                point: {
                    pixelSize: 12,
            ...
            // 创建连接线
            const connectionLine = this.viewer.entities.add({
                id: `sightline-connection-${Date.now()}`,
                polyline: {
                    positions: [startPoint, endPoint],
                    width: 3,
                    material: SuperMap3D.Color.CYAN,
                    clampToGround: false
                }
            });
            ...
            const analysisData = {
                ...
                entities: [observerMarker, targetMarker, connectionLine],
            ...
            if (typeof window !== 'undefined' && window.showSuccessMessage) {
                window.showSuccessMessage('通视分析完成', `距离: ${distance.toFixed(2)}m (SuperMap3D原生实现)`);
            }
    ...
    calculatePolygonPerimeter(positions) {
        ...
        for (let i = 0; i < positions.length; i++) {
            ...
            perimeter += SuperMap3D.Cartesian3.distance(positions[i], positions[nextIndex]);
        }
        return perimeter;
    }
    ...
    arePointsCollinear(positions) {
        ...
        // 计算向量叉积
        const v1 = SuperMap3D.Cartesian3.subtract(p2, p1, new SuperMap3D.Cartesian3());
        const v2 = SuperMap3D.Cartesian3.subtract(p3, p1, new SuperMap3D.Cartesian3());
        const cross = SuperMap3D.Cartesian3.cross(v1, v2, new SuperMap3D.Cartesian3());
    ...
    performProfileAnalysis(positions, options = {}) {
        try {
            console.log('📊 开始剖面分析...', { pointCount: positions ? positions.length : 0 });
            ...
            if (totalDistance > 50000) {
                console.warn('⚠️ 剖面线距离较长(' + totalDistance.toFixed(2) + 'm)，分析可能耗时较长');
            }
            ...
            // 设置分析参数
            const sampleCount = options.sampleCount || Math.min(Math.max(Math.floor(totalDistance / 10), 100), 1000);
            ...
            console.log(`🔍 采样点数: ${sampleCount}, 剖面线长度: ${this.formatDistance(totalDistance)}`);
            ...
            try {
                ...
                if (positions.length >= 2) {
                    ...
                    const startLongitude = SuperMap3D.Math.toDegrees(startCartographic.longitude);
                    const startLatitude = SuperMap3D.Math.toDegrees(startCartographic.latitude);
                    const startHeight = startCartographic.height;
                    
                    const endLongitude = SuperMap3D.Math.toDegrees(endCartographic.longitude);
                    const endLatitude = SuperMap3D.Math.toDegrees(endCartographic.latitude);
            ...
            if (profile) {
                ...
                // 生成剖面线上的采样点
                const samplePoints = this.generateProfileSamplePoints(positions, sampleCount);
                ...
                if (showElevationLabels && elevationData.length > 0) {
                    const labelInterval = Math.max(1, Math.floor(elevationData.length / 15));
    ...
    displayProfileChart(profile, buffer) {
        try {
            ...
            console.log('🔍 检查主界面元素:', {
                ...
                mainContainerDisplay: chartContainer ? chartContainer.style.display : 'N/A'
            });
    ...
    hideProfileChart() {
        try {
            // 隐藏主界面的剖面图表容器
            const mainChartContainer = document.getElementById('mainProfileChartContainer');
    ...
    generateProfileSamplePoints(positions, sampleCount) {
        ...
        for (let i = 0; i < positions.length - 1; i++) {
            const segmentLength = SuperMap3D.Cartesian3.distance(positions[i], positions[i + 1]);
        ...
        // 按比例在每段上分配采样点
        for (let i = 0; i < positions.length - 1; i++) {
            const segmentSampleCount = Math.max(1, Math.floor(sampleCount * segments[i] / totalLength));
            
            for (let j = 0; j < segmentSampleCount; j++) {
                const t = j / segmentSampleCount;
                const point = SuperMap3D.Cartesian3.lerp(positions[i], positions[i + 1], t, new SuperMap3D.Cartesian3());
                points.push(point);
            }
        }
        
        // 确保包含最后一个点
        if (points.length === 0 || !SuperMap3D.Cartesian3.equals(points[points.length - 1], positions[positions.length - 1])) {
    ...
    getElevationProfile(samplePoints) {
        ...
        try {
            for (let i = 0; i < samplePoints.length; i++) {
                ...
                // 计算累积距离
                if (i > 0) {
                    cumulativeDistance += SuperMap3D.Cartesian3.distance(samplePoints[i - 1], point);
                }
    ...
    queryModelAttributes(windowPosition, options = {}) {
        try {
            ...
            // 获取世界坐标
            const cartesian = this.viewer.camera.pickEllipsoid(windowPosition, this.scene.globe.ellipsoid);
            if (cartesian) {
                result.worldPosition = cartesian;
                const cartographic = SuperMap3D.Cartographic.fromCartesian(cartesian);
                result.coordinates = {
                    longitude: SuperMap3D.Math.toDegrees(cartographic.longitude).toFixed(finalOptions.precision),
                    latitude: SuperMap3D.Math.toDegrees(cartographic.latitude).toFixed(finalOptions.precision),
            ...
            // 显示弹窗
            if (finalOptions.showPopup && result.properties && Object.keys(result.properties).length > 0) {
    ...
    initMeasurementTools() {
        try {
            ...
            // 初始化量测相关的事件处理器
            this.measurementHandler = new SuperMap3D.ScreenSpaceEventHandler(this.scene.canvas);
    ...
    initProfileAnalysis() {
        try {
            ...
            // 初始化剖面分析的事件处理器
            this.profileHandler = new SuperMap3D.ScreenSpaceEventHandler(this.scene.canvas);
    ...
    getAnalysisCapabilities() {
        ...
        // 统计可用功能
        const availableCount = Object.values(capabilities).filter(cap => cap.available).length;
        const totalCount = Object.keys(capabilities).length;
        const nativeCount = Object.values(capabilities).filter(cap => cap.available && cap.native).length;

        console.log(`\n📊 数字孪生分析功能状态报告:`);
        console.log(`总功能数: ${totalCount}`);
        console.log(`可用功能: ${availableCount}/${totalCount} (${Math.round(availableCount/totalCount*100)}%)`);
        console.log(`原生API: ${nativeCount}/${availableCount} (${availableCount > 0 ? Math.round(nativeCount/availableCount*100) : 0}%)`);
        console.log(`自定义实现: ${availableCount - nativeCount}/${availableCount}\n`);
        ...
        return {
            ...
            summary: {
                ...
                nativeRate: availableCount > 0 ? Math.round(nativeCount/availableCount*100) : 0
            }
        };
    }
    ...
    exportAllAnalysisResults(options = {}) {
        try {
            ...
            console.log(`批量导出完成: ${Object.keys(exportResults).length}/${this.analysisResults.size} 个分析结果`);
            ...
            switch (analysisType) {
                case 'viewshed':
                    ...
                    if (result.viewshed && !result.isDestroyed) {
                        try {
                            ...
                            try {
                                isDestroyed = result.viewshed.isDestroyed && result.viewshed.isDestroyed();
                            } catch (checkError) {
                            ...
                            if (!isDestroyed) {
                                // 根据保存的集合信息进行精确清理
                                if (result.addedToCollection === 'viewshedAnalysis3Ds' && this.scene.viewshedAnalysis3Ds) {
                                    try {
                                        this.scene.viewshedAnalysis3Ds.remove(result.viewshed);
                                        console.log('✅ 从viewshedAnalysis3Ds集合中移除可视域分析');
                                    } catch (removeError) {
                                        console.warn('从viewshedAnalysis3Ds集合移除失败:', removeError.message);
                                    }
                                } else if (result.addedToCollection === 'primitives' && this.scene.primitives) {
                                    try {
                                        this.scene.primitives.remove(result.viewshed);
                                        console.log('✅ 从primitives集合中移除可视域分析');
                                    } catch (removeError) {
                                        console.warn('从primitives集合移除失败:', removeError.message);
                                    }
                                } else {
                                    // 兜底策略：尝试所有可能的集合
                                    let removed = false;
                                    if (this.scene.viewshedAnalysis3Ds && typeof this.scene.viewshedAnalysis3Ds.remove === 'function') {
                                        try {
                                            this.scene.viewshedAnalysis3Ds.remove(result.viewshed);
                                            removed = true;
                                            console.log('✅ 兜底策略：从viewshedAnalysis3Ds集合中移除');
                                        } catch (e) { 
                                            console.warn('兜底策略：从viewshedAnalysis3Ds集合移除失败:', e.message);
                                        }
                                    }
                                    if (!removed && this.scene.primitives && typeof this.scene.primitives.remove === 'function') {
                                        try {
                                            this.scene.primitives.remove(result.viewshed);
                                            console.log('✅ 兜底策略：从primitives集合中移除');
                                        } catch (e) { 
                                            console.warn('兜底策略：从primitives集合移除失败:', e.message);
                                        }
                                    }
                                }
                                ...
                                if (typeof result.viewshed.destroy === 'function') {
                                    try {
                                        ...
                                        console.warn('销毁可视域分析对象失败:', destroyError.message);
                    ...
                    if (result.officialViewshed && !result.officialViewshedDestroyed) {
                        try {
                            ...
                            let isDestroyed = false;
                            try {
                                isDestroyed = result.officialViewshed.isDestroyed && result.officialViewshed.isDestroyed();
                            } catch (checkError) {
                                console.warn('检查官方可视域分析对象状态失败:', checkError.message);
                                isDestroyed = true; // 假设已销毁
                            }
                            ...
                            if (!isDestroyed) {
                                ...
                                if (this.scene.viewshedAnalysis3Ds && typeof this.scene.viewshedAnalysis3Ds.remove === 'function') {
                                    try {
                                        this.scene.viewshedAnalysis3Ds.remove(result.officialViewshed);
                                        console.log('✅ 从viewshedAnalysis3Ds集合中移除官方可视域分析');
                                    } catch (removeError) {
                                        console.warn('从viewshedAnalysis3Ds集合移除官方可视域分析失败:', removeError.message);
                                    }
                                }
                                ...
                                if (typeof result.officialViewshed.destroy === 'function') {
                                    try {
                                        ...
                                        console.warn('销毁官方可视域分析对象失败:', destroyError.message);
                    ...
                    if (result.viewshed3D && !result.isDestroyed) {
                        try {
                            ...
                            let isDestroyed = false;
                            try {
                                isDestroyed = result.viewshed3D.isDestroyed && result.viewshed3D.isDestroyed();
                            } catch (checkError) {
                            ...
                            if (!isDestroyed) {
                                ...
                                if (this.scene.viewshedAnalysis3Ds && typeof this.scene.viewshedAnalysis3Ds.remove === 'function') {
                                    try {
                                        ...
                                        console.log('✅ 从viewshedAnalysis3Ds集合中移除viewshed3D对象');
                                    } catch (removeError) {
                                        console.warn('从viewshedAnalysis3Ds集合移除viewshed3D对象失败:', removeError.message);
                                    }
                                }
                                ...
                                if (typeof result.viewshed3D.destroy === 'function') {
                                    try {
                                        ...
                                        console.warn('销毁viewshed3D对象失败:', destroyError.message);
                                        result.isDestroyed = true; // 标记为已销毁，避免重复尝试
                                    }
                                }
                            } else {
                    ...
                    if (result.analysis) {
                        try {
                            // 检查备用分析对象是否已经被销毁
                            const isAnalysisDestroyed = result.analysis.isDestroyed && result.analysis.isDestroyed();
                            
                            if (!isAnalysisDestroyed) {
                                if (this.scene.viewshedAnalysis3Ds && typeof this.scene.viewshedAnalysis3Ds.remove === 'function') {
                                    this.scene.viewshedAnalysis3Ds.remove(result.analysis);
                                } else if (this.scene.primitives && typeof this.scene.primitives.remove === 'function') {
                    ...
                    // 清除观察点标记 - 支持多种存储方式
                    if (result.observerEntity && this.viewer.entities.contains(result.observerEntity)) {
                    ...
                    if (result.entities && Array.isArray(result.entities)) {
                        result.entities.forEach(entity => {
                            if (this.viewer.entities.contains(entity)) {
                                ...
                                console.log('🗑️ 清除可视域分析标记:', entity.label ? entity.label.text._value : '未命名标记');
                            }
                        });
                    }
                    ...
                    if (result.clipRegions && Array.isArray(result.clipRegions)) {
                        result.clipRegions.forEach(clipRegion => {
                            // 清除裁剪面实体
                            if (clipRegion.entity && this.viewer.entities.contains(clipRegion.entity)) {
                            ...
                            // 从可视域分析对象中移除裁剪面
                            try {
                                if (result.officialViewshed && typeof result.officialViewshed.removeClipRegion === 'function') {
                                    result.officialViewshed.removeClipRegion(clipRegion.name);
                                    console.log('✅ 从官方ViewShed3D对象移除裁剪面:', clipRegion.name);
                                } else if (result.viewshed3D && typeof result.viewshed3D.removeClipRegion === 'function') {
                                    result.viewshed3D.removeClipRegion(clipRegion.name);
                                    console.log('✅ 从自定义可视域分析对象移除裁剪面:', clipRegion.name);
                                } else if (result.viewshed && typeof result.viewshed.removeClipRegion === 'function') {
                                    result.viewshed.removeClipRegion(clipRegion.name);
                                    console.log('✅ 从备用可视域分析对象移除裁剪面:', clipRegion.name);
                                }
                            } catch (removeError) {
                    ...
                    // 清除所有裁剪面（兜底策略）
                    try {
                        if (result.officialViewshed && typeof result.officialViewshed.removeAllClipRegion === 'function') {
                            result.officialViewshed.removeAllClipRegion();
                            console.log('✅ 清除官方ViewShed3D对象的所有裁剪面');
                        } else if (result.viewshed3D && typeof result.viewshed3D.removeAllClipRegion === 'function') {
                            result.viewshed3D.removeAllClipRegion();
                            console.log('✅ 清除自定义可视域分析对象的所有裁剪面');
                        } else if (result.viewshed && typeof result.viewshed.removeAllClipRegion === 'function') {
                    ...
                    try {
                        ...
                        if (this.scene.viewshedAnalysis3Ds) {
                            ...
                            for (let i = viewshedCount - 1; i >= 0; i--) {
                                try {
                                    ...
                                    console.warn(`强制清除可视域分析对象 ${i} 失败:`, destroyError.message);
                                }
                            }
                            // 清空集合
                            if (typeof this.scene.viewshedAnalysis3Ds.removeAll === 'function') {
                                this.scene.viewshedAnalysis3Ds.removeAll();
                        ...
                        this.viewer.entities.values.forEach(entity => {
                            if (entity.id && (entity.id.includes('viewshed') || entity.id.includes('observer'))) {
                        ...
                        if (this.scene.primitives) {
                            ...
                            for (let i = 0; i < this.scene.primitives.length; i++) {
                                const primitive = this.scene.primitives.get(i);
                                // 检查是否为可视域相关的图形
                                if (primitive && (primitive._viewshed || primitive.viewshed || 
                                    (primitive.constructor && primitive.constructor.name && 
                                     primitive.constructor.name.toLowerCase().includes('viewshed')))) {
                            ...
                            primitivesToRemove.forEach(primitive => {
                                try {
                                    ...
                                    console.warn('强制清除primitives中的可视域图形失败:', removeError.message);
                                }
                            });
                        }
                        ...
                        console.warn('⚠️ 可视域分析强制清除过程中出错:', forceCleanError.message);
                    }
                ...
                case 'sightline':
                    ...
                    for (let i = 0; i < sightlineResults.length; i++) {
                        ...
                        // 清除官方Sightline对象
                        if (sightlineResult.sightlineObject && sightlineResult.implementation === 'official') {
                            try {
                                // 清除目标点
                                if (sightlineResult.targetName) {
                                    sightlineResult.sightlineObject.removeTargetPoint(sightlineResult.targetName);
                                    console.log(`✅ 清除官方通视分析目标点 ${i + 1}:`, sightlineResult.targetName);
                                }
                                
                                // 清除所有目标点（兜底策略）
                                if (typeof sightlineResult.sightlineObject.removeAllTargetPoint === 'function') {
                    ...
                    for (let i = 0; i < sightlineResults.length; i++) {
                        ...
                        if (sightlineResult.sightline && !sightlineResult.isDestroyed) {
                            try {
                                ...
                                let isDestroyed = false;
                                try {
                                    isDestroyed = sightlineResult.sightline.isDestroyed && sightlineResult.sightline.isDestroyed();
                                } catch (checkError) {
                                    console.warn(`检查通视分析对象 ${i + 1} 状态失败:`, checkError.message);
                                    isDestroyed = true; // 假设已销毁
                                }
                                
                                if (!isDestroyed) {
                                    // 根据保存的集合信息进行精确清理
                                    if (sightlineResult.addedToCollection === 'sightlineAnalysis' && this.scene.sightlineAnalysis) {
                                        try {
                                            this.scene.sightlineAnalysis.remove(sightlineResult.sightline);
                                            console.log(`✅ 从sightlineAnalysis集合中移除通视分析 ${i + 1}`);
                                        } catch (removeError) {
                                            console.warn(`从sightlineAnalysis集合移除 ${i + 1} 失败:`, removeError.message);
                                        }
                                    } else if (sightlineResult.addedToCollection === 'primitives' && this.scene.primitives) {
                                        try {
                                            this.scene.primitives.remove(sightlineResult.sightline);
                                            console.log(`✅ 从primitives集合中移除通视分析 ${i + 1}`);
                                        } catch (removeError) {
                                            console.warn(`从primitives集合移除 ${i + 1} 失败:`, removeError.message);
                                        }
                                    } else {
                                        // 兜底策略：尝试所有可能的集合
                                        let removed = false;
                                        if (this.scene.sightlineAnalysis && typeof this.scene.sightlineAnalysis.remove === 'function') {
                                            try {
                                                this.scene.sightlineAnalysis.remove(sightlineResult.sightline);
                                                removed = true;
                                                console.log(`✅ 兜底策略：从sightlineAnalysis集合中移除 ${i + 1}`);
                                            } catch (e) { 
                                                console.warn(`兜底策略：从sightlineAnalysis集合移除 ${i + 1} 失败:`, e.message);
                                            }
                                        }
                                        if (!removed && this.scene.primitives && typeof this.scene.primitives.remove === 'function') {
                                            try {
                                                this.scene.primitives.remove(sightlineResult.sightline);
                                                console.log(`✅ 兜底策略：从primitives集合中移除 ${i + 1}`);
                                            } catch (e) { 
                                                console.warn(`兜底策略：从primitives集合移除 ${i + 1} 失败:`, e.message);
                                            }
                                        }
                                    }
                                    ...
                                    if (typeof sightlineResult.sightline.destroy === 'function') {
                                        try {
                                            ...
                                            console.warn(`销毁通视分析对象 ${i + 1} 失败:`, destroyError.message);
                                ...
                                console.warn(`⚠️ 清除通视分析对象 ${i + 1} 时出错:`, removeError);
                                sightlineResult.isDestroyed = true; // 标记为已销毁，避免重复尝试
                            }
                    ...
                    for (let i = 0; i < sightlineResults.length; i++) {
                        ...
                        if (sightlineResult.analysis) {
                            try {
                                // 检查备用分析对象是否已经被销毁
                                const isAnalysisDestroyed = sightlineResult.analysis.isDestroyed && sightlineResult.analysis.isDestroyed();
                                
                                if (!isAnalysisDestroyed) {
                                    if (this.scene.sightlineAnalysis && typeof this.scene.sightlineAnalysis.remove === 'function') {
                                        this.scene.sightlineAnalysis.remove(sightlineResult.analysis);
                                    } else if (this.scene.primitives && typeof this.scene.primitives.remove === 'function') {
                                ...
                                console.warn(`⚠️ 清除备用通视分析对象 ${i + 1} 时出错:`, removeError);
                            }
                        }
                        
                        // 清除起点和终点标记 - 支持多种存储方式
                        if (sightlineResult.startMarker && this.viewer.entities.contains(sightlineResult.startMarker)) {
                            this.viewer.entities.remove(sightlineResult.startMarker);
                            console.log(`✅ 清除通视分析起点标记 ${i + 1}`);
                        }
                        if (sightlineResult.endMarker && this.viewer.entities.contains(sightlineResult.endMarker)) {
                            this.viewer.entities.remove(sightlineResult.endMarker);
                            console.log(`✅ 清除通视分析终点标记 ${i + 1}`);
                        }
                        // 清除存储在entities数组中的标记
                        if (sightlineResult.entities && Array.isArray(sightlineResult.entities)) {
                            sightlineResult.entities.forEach(entity => {
                                if (this.viewer.entities.contains(entity)) {
                                    this.viewer.entities.remove(entity);
                                    console.log(`🗑️ 清除通视分析标记 ${i + 1}:`, entity.label ? entity.label.text._value : '未命名标记');
                                }
                            });
                        }
                    }
                ...
                case 'profile':
                    ...
                    if (result.profile && !result.profile.isCustom) {
                        try {
                            ...
                            } else if (this.scene.profileAnalysis && typeof this.scene.profileAnalysis.remove === 'function') {
    ...
    isAnalysisRelatedEntity(entity) {
        ...
        if (entity.point || entity.billboard) {
            ...
            // 但需要排除明确的模型标记
            if (!entity.model && !entity.properties?.isModel && !entity.name?.includes('模型')) {
                // 进一步检查是否为用户添加的分析点
                // 检查创建时间或其他标识
                if (entity.properties?.createdBy === 'analysis' || 
                    entity.properties?.type === 'analysisPoint' ||
                    !entity.properties?.isSystemEntity) { // 非系统实体
                    return true;
                }
                ...
                if (entity.point && entity.point.color) {
                    ...
                    if (color && (color.red > 0.8 || color.yellow > 0.8)) { // 红色或黄色点通常是分析点
                        return true;
                    }
                }
            }
        ...
        if (entity.entityCollection && entity.entityCollection.id) {
            ...
            if (collectionId.includes('analysis') || 
            ...
            entities.forEach(entity => {
                ...
                if (entity.point || entity.billboard) {
                    ...
                    if (!entity.model && 
                        ...
                        if (!entity.properties?.isSystemEntity && 
                ...
                if (entity.properties?.createdAt) {
                    ...
                    if (timeDiff < 24 * 60 * 60 * 1000 && (entity.point || entity.billboard)) {
                        shouldRemove = true;
                    }
                }
                ...
                if (entity.point && entity.point.color) {
                    ...
                    if (color) {
                        // 红色、黄色、绿色、蓝色等常用分析颜色
                        if ((color.red > 0.7 && color.green < 0.3 && color.blue < 0.3) || // 红色
                            (color.red > 0.7 && color.green > 0.7 && color.blue < 0.3) || // 黄色
                            (color.red < 0.3 && color.green > 0.7 && color.blue < 0.3) || // 绿色
                            (color.red < 0.3 && color.green < 0.3 && color.blue > 0.7)) { // 蓝色
                            shouldRemove = true;
                        }
                    }
                }
            ...
            pointsToRemove.forEach(entity => {
                try {
                    if (this.viewer.entities.contains(entity)) {
                        this.viewer.entities.remove(entity);
                        removedCount++;
                        console.log('🗑️ 清除分析点:', entity.label?.text?._value || entity.name || '未命名点');
                    }
                } catch (error) {
    ...
    clearAllAnalysis() {
        try {
            ...
            if (this.analysisResults) {
                if (this.analysisResults instanceof Map && typeof this.analysisResults.forEach === 'function') {
                    ...
                    console.warn('⚠️ analysisResults类型不正确:', typeof this.analysisResults);
                    // 重新初始化为Map
                    this.analysisResults = new Map();
                }
            } else {
            ...
            if (this.measurementEntities && Array.isArray(this.measurementEntities)) {
                this.measurementEntities.forEach(entity => {
                    if (entity && this.viewer && this.viewer.entities && this.viewer.entities.contains(entity)) {
                        this.viewer.entities.remove(entity);
            ...
            const entitiesToRemove = [];
            if (this.viewer && this.viewer.entities && this.viewer.entities.values && Array.isArray(this.viewer.entities.values)) {
                this.viewer.entities.values.forEach(entity => {
            ...
            if (entitiesToRemove && Array.isArray(entitiesToRemove)) {
                entitiesToRemove.forEach(entity => {
                    if (entity && this.viewer && this.viewer.entities) {
                        try {
                            this.viewer.entities.remove(entity);
                            console.log('🗑️ 清除分析实体:', entity.label ? entity.label.text._value : entity.name || '未命名');
                        } catch (removeError) {
            ...
            this.activeAnalysis = null;
            console.log('✅ 智能清除完成，共清除', entitiesToRemove.length, '个分析实体，模型已保留');
    ...
    getAnalysisUnits(analysisType) {
        const units = {
            ...
            excavation: { area: 'square_meters', volume: 'cubic_meters', depth: 'meters' },
                    ...
                    if (analysisData.positions && analysisData.positions.length > 1) {
                        ...
                        geojson.features.push({
                            ...
                            properties: {
                                ...
                                ...(analysisData.elevationProfile && { elevationProfile: analysisData.elevationProfile })
                            }
                        });
                    }
                    break;
                    ...
                    if (analysisData.positions && analysisData.positions.length > 2) {
                        ...
                        geojson.features.push({
                            ...
                            properties: {
                                ...
                                ...(analysisData.waterLevel && { waterLevel: analysisData.waterLevel })
                            }
                        });
                    }
                    break;
    ...
    initInteractiveManager() {
        try {
            if (typeof InteractiveAnalysisManager !== 'undefined') {
                this.interactiveManager = new InteractiveAnalysisManager(this.viewer, this.scene, this);
    ...
    setSightlineViewPoint(position) {
        try {
            ...
            // 转换为经纬度坐标
            const cartographic = this.cartesianToDegrees(position);
            
            if (this.sightline) {
                // 设置观察点
                this.sightline.viewPosition = cartographic;
                console.log('✅ 原生通视分析观察点已设置:', cartographic);
            }
    ...
    addSightlineTargetPoint(position) {
        try {
            ...
            if (this.sightline) {
                // 添加目标点到原生通视分析
                success = this.sightline.addTargetPoint({
                    position: cartographic,
                    name: pointName
                });
                
                if (success) {
                    console.log('✅ 原生通视分析目标点已添加:', pointName, cartographic);
                } else {
                    console.warn('⚠️ 原生通视分析目标点添加失败');
                }
            }
            ...
            console.log('✅ 目标点标记已添加:', pointName);
            return true;
        } catch (error) {
    ...
    clearSightlineAnalysis() {
        try {
            ...
            const sightlineObjects = [this.sightline, this.sightlineAnalysis].filter(obj => obj);
            ...
            sightlineObjects.forEach((sightlineObj, index) => {
                try {
                    ...
                    console.warn(`⚠️ 清除原生通视分析对象 ${index + 1} 时出错:`, nativeError);
                }
            });
            ...
            // 清除目标点标记
            if (this.sightlineTargetPointEntities && this.sightlineTargetPointEntities.length > 0) {
            ...
            this.viewer.entities.values.forEach(entity => {
                ...
                if (entity.polyline && !entity.label) {
                    // 进一步检查是否可能是通视线条
                    if (entity.polyline.positions && entity.polyline.positions._value && 
    ...
    isValidCartesian3(cartesian3) {
        ...
        return isFinite(cartesian3.x) && isFinite(cartesian3.y) && isFinite(cartesian3.z) &&
               !isNaN(cartesian3.x) && !isNaN(cartesian3.y) && !isNaN(cartesian3.z);
    }
    ...
    isValidCartographic(cartographic) {
        ...
        return isFinite(cartographic.longitude) && isFinite(cartographic.latitude) && isFinite(cartographic.height) &&
               !isNaN(cartographic.longitude) && !isNaN(cartographic.latitude) && !isNaN(cartographic.height);
    }
    ...
    isValidLonLatHeight(lonLatHeight) {
        ...
        return isFinite(lon) && isFinite(lat) && isFinite(height) &&
        ...
        try {
                ...
                try {
                    switch (property) {
                        ...
                        case 'verticalFov':
                            ...
                            break;
                        case 'visibleColor':
                            const visibleColor = this.hexToColor(value, 0.5);
                            viewshedResult.viewshed3D.visibleAreaColor = visibleColor;
                            break;
                        case 'invisibleColor':
                            const invisibleColor = this.hexToColor(value, 0.5);
            ...
            if (viewshedResult.analysis && !viewshedResult.isDestroyed) {
                try {
                    // 对于自定义实现，需要重新执行分析
                    const options = {
                        horizontalFov: property === 'horizontalFov' ? value : viewshedResult.horizontalFov,
                        verticalFov: property === 'verticalFov' ? value : viewshedResult.verticalFov,
                        visibleColor: property === 'visibleColor' ? this.hexToColor(value, 0.5) : viewshedResult.visibleColor,
                        hiddenColor: property === 'invisibleColor' ? this.hexToColor(value, 0.5) : viewshedResult.hiddenColor
                    };

                    const distance = property === 'distance' ? value : viewshedResult.distance;
                    const pitch = property === 'pitch' ? value : viewshedResult.pitch;
                    const heading = property === 'direction' ? value : viewshedResult.heading;

                    // 清除当前分析结果
                    this.clearAnalysisResults('viewshed');

                    // 重新执行可视域分析
                    this.performViewshedAnalysis(viewshedResult.viewPoint, distance, pitch, heading, options);
    ...
    setViewshedClipMode(mode) {
        try {
            ...
            let clipMode;
            if (mode === 'keep-inside') {
                clipMode = SuperMap3D.ClippingType ? SuperMap3D.ClippingType.KeepInside : 0;
            } else if (mode === 'keep-outside') {
                clipMode = SuperMap3D.ClippingType ? SuperMap3D.ClippingType.KeepOutside : 1;
            } else {
            ...
            if (viewshedResult.viewshed3D && !viewshedResult.isDestroyed) {
                try {
                    ...
                    console.warn(`⚠️ 设置官方ViewShed3D对象裁剪模式失败: ${setError.message}`);
                }
            }
    ...
    startDrawClipPlane() {
        try {
            ...
            this.handler.setInputAction((event) => {
                ...
                this.scene.pickPositionAsync(event.position).then((position) => {
                    if (position) {
            ...
            this.scene.layers.layerQueue.forEach(layer => {
                if (layer && typeof layer.setCustomClipPlane === 'function') {
                    try {
                        ...
                        console.warn('⚠️ 图层裁剪失败:', layer.name || '未命名图层', layerError);
                    }
                }
            });
    ...
    drawViewshedClipPlane(positions) {
        try {
            ...
            for (let i = 0; i < positions.length; i++) {
                ...
                // 避免重复坐标
                if (clipPositions.indexOf(longitude) === -1 && clipPositions.indexOf(latitude) === -1) {
                    clipPositions.push(longitude);
                    clipPositions.push(latitude);
                    clipPositions.push(height);
                }
            ...
            if (viewshedResult) {
                try {
                    // 生成唯一的裁剪面名称
                    const clipRegionName = 'clipRegion_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9);
                    
                    // 优先使用官方ViewShed3D对象
                    if (viewshedResult.officialViewshed && typeof viewshedResult.officialViewshed.addClipRegion === 'function') {
                    ...
                    // 备用：使用自定义可视域分析对象
                    else if (viewshedResult.viewshed3D && typeof viewshedResult.viewshed3D.addClipRegion === 'function') {
                        viewshedResult.viewshed3D.addClipRegion({
                            name: clipRegionName,
                            position: clipPositions
                        });
                        console.log('✅ 使用自定义可视域分析对象应用裁剪面:', clipRegionName);
                    }
                    // 兜底：使用viewshed对象
                    else if (viewshedResult.viewshed && typeof viewshedResult.viewshed.addClipRegion === 'function') {
    ...
    performShadowAnalysis(options = {}) {
        try {
            ...
            // 创建绘制处理器
            this.shadowDrawHandler = new SuperMap3D.DrawHandler(this.viewer, SuperMap3D.DrawMode.Polygon, 0);
    ...
    handleShadowDrawComplete(result, options) {
        try {
            ...
            // 去除重复点
            const uniquePositions = SuperMap3D.arrayRemoveDuplicates(positions, SuperMap3D.Cartesian3.equalsEpsilon);
            const points = [];
            
            // 遍历多边形，取出所有点
            for (let i = 0, len = uniquePositions.length; i < len; i++) {
                // 转化为经纬度，并加入至临时数组
                const cartographic = SuperMap3D.Cartographic.fromCartesian(uniquePositions[i]);
                const longitude = SuperMap3D.Math.toDegrees(cartographic.longitude);
                const latitude = SuperMap3D.Math.toDegrees(cartographic.latitude);
        ...
        try {
            ...
            this.sunlightInterval = setInterval(() => {
                if (currentHour < endHour) {
                    ...
                    this.viewer.clock.currentTime = SuperMap3D.JulianDate.fromDate(currentTime);
        ...
        try {
            ...
            handler.setInputAction((event) => {
                this.scene.pickPositionAsync(event.position).then((position) => {
                    if (!position) {
                        console.warn('⚠️ 无法获取点击位置，请点击模型表面');
                        if (callback) callback({ error: '无法获取点击位置，请点击模型表面', success: false });
                        return;
                    }
                    
                    const cartographic = SuperMap3D.Cartographic.fromCartesian(position);
                    
                    this.shadowQuery.getShadowRadioAsync(cartographic).then((shadowRatio) => {
                        const longitude = SuperMap3D.Math.toDegrees(cartographic.longitude);
                        const latitude = SuperMap3D.Math.toDegrees(cartographic.latitude);
                        const height = cartographic.height;
                        ...
                        if (callback) {
                            callback({ error: '获取阴影率失败: ' + error.message, success: false });
                        }
                    });
                }).catch((error) => {
                    console.error('❌ 获取位置失败:', error);
                    if (callback) {
                        callback({ error: '获取位置失败: ' + error.message, success: false });
                    }
                });
            }, SuperMap3D.ScreenSpaceEventType.LEFT_CLICK);
            ...
            console.log('👆 点击地图获取阴影率（绿色=低阴影率，黄色=中等阴影率，红色=高阴影率）');
            
        } catch (error) {
            console.error('❌ 获取阴影率功能启动失败:', error);
            if (callback) {
                callback({ error: '获取阴影率功能启动失败: ' + error.message, success: false });
            }
        }
    }
    ...
    initSkylineAnalysis() {
        try {
            ...
            // 创建多边形绘制处理器（用于限高体绘制）
            this.skylinePolygonHandler = new SuperMap3D.DrawHandler(this.viewer, SuperMap3D.DrawMode.Polygon);
            
            // 绑定多边形绘制完成事件
            this.skylinePolygonHandler.drawEvt.addEventListener((result) => {
                this.handleSkylinePolygonDrawComplete(result);
            });
     ...
     * 处理限高体多边形绘制完成
     */
    handleSkylinePolygonDrawComplete(result) {
        try {
            // 清除之前的限高体对象
            if (this.skyline) {
            ...
            for (let i = 0, len = pos.length; i < len; i++) {
                // 转化为经纬度，并加入至临时数组
                const cartographic = SuperMap3D.Cartographic.fromCartesian(pos[i]);
                const longitude = SuperMap3D.Math.toDegrees(cartographic.longitude);
                const latitude = SuperMap3D.Math.toDegrees(cartographic.latitude);
            ...
            // 再次遍历转化为接口所需的数组格式
            for (let i = 0, len = uniquePositions.length; i < len; i++) {
                arr.push(uniquePositions[i][0]);
                arr.push(uniquePositions[i][1]);
            }
            ...
            console.error('❌ 处理限高体绘制完成事件失败:', error);
        }
    }
    ...
    extractSkyline(radius = 10000) {
        try {
            ...
            this.skyline.direction = SuperMap3D.Math.toDegrees(this.scene.camera.heading);
    ...
    drawLimitBody() {
        try {
            ...
            if (this.skylinePolygonHandler.active) {
                console.log('⚠️ 限高体绘制已激活');
                return;
            }
            ...
            this.skylinePolygonHandler.activate();
            console.log('✅ 开始绘制限高体');
        } catch (error) {
    ...
    getSkylineArea() {
        try {
            ...
            const points = this.skyline.getSkyline3D();
            const pointArr = [];
            const cameraPoint = SuperMap3D.Cartesian3.fromDegrees(longitude, latitude, height);
            pointArr.push(cameraPoint);
            
            for (let i = 0; i < points.x.length; i++) {
                const point = SuperMap3D.Cartesian3.fromDegrees(points.x[i], points.y[i], points.z[i]);
                pointArr.push(point);
            }
            ...
            const entity = this.viewer.entities.add({
                polygon: {
                    ...
                    material: SuperMap3D.Color.ORANGE.withAlpha(0.8)
                }
            });
            
            // 记录实体以便清理
            this.analysisEntities.push(entity);
            
            console.log('✅ 天际线拉伸闭合体创建完成');
            return entity;
        } catch (error) {
            console.error('❌ 获取天际线拉伸闭合体失败:', error);
            return null;
        }
    }
    ...
    clearSkylineAnalysis() {
        try {
            ...
            if (this.skylinePolygonHandler) {
                try {
                    this.skylinePolygonHandler.clear();
                    this.skylinePolygonHandler.deactivate();
                    if (typeof this.skylinePolygonHandler.destroy === 'function') {
                        this.skylinePolygonHandler.destroy();
                    }
                } catch (e) {
                    console.warn('清除多边形绘制处理器时出错:', e);
                }
            ...
            // 清除所有包含天际线关键词的实体
            const entitiesToRemove = [];
            const skylineKeywords = [
                '天际线', 'skyline', 'Skyline', 'SKYLINE',
                '限高体', 'limitbody', 'LimitBody',
                '天际线拉伸', '天际线闭合体', '天际线分析',
                'skylineAnalysis', 'skylineArea', 'skylinePolygon'
            ];
            
            this.viewer.entities.values.forEach(entity => {
                if (entity.name) {
                    const shouldRemove = skylineKeywords.some(keyword => 
                ...
                if (entity.id && typeof entity.id === 'string') {
                    const shouldRemove = skylineKeywords.some(keyword => 
                        entity.id.toLowerCase().includes(keyword.toLowerCase())
                    );
                    if (shouldRemove) {
                        entitiesToRemove.push(entity);
                    }
            ...
            entitiesToRemove.forEach(entity => {
                try {
                    this.viewer.entities.remove(entity);
                    console.log('🗑️ 清除天际线实体:', entity.name || entity.id || '未命名');
                } catch (e) {
                    console.warn('移除天际线实体时出错:', e);
                }
            });
            
            // 清除所有可能的天际线图表容器
            const chartContainerIds = ['skylineChart', 'skyline-chart', 'skylineChartContainer', 'chart-container'];
            ...
            if (this.viewer) {
                ...
                try {
                    ...
                    console.warn('清除canvas事件监听器时出错:', e);
                }
            }
            
            console.log('✅ 天际线分析清除完成，共清除', entitiesToRemove.length, '个相关实体');
        } catch (error) {
    ...
    async selectOptimalViewPoint() {
        try {
            ...
            const cameraCartographic = SuperMap3D.Cartographic.fromCartesian(cameraPosition);
            
            if (cameraCartographic) {
                const longitude = SuperMap3D.Math.toDegrees(cameraCartographic.longitude);
            ...
            for (const point of samplePoints) {
                try {
                    ...
                    if (pickedPosition && SuperMap3D.defined(pickedPosition.x)) {
                        const cartographic = SuperMap3D.Cartographic.fromCartesian(pickedPosition);
                        
                        if (cartographic && !isNaN(cartographic.height) && cartographic.height > 0) {
                            const result = {
                                longitude: SuperMap3D.Math.toDegrees(cartographic.longitude),
                                latitude: SuperMap3D.Math.toDegrees(cartographic.latitude),
                                height: cartographic.height + 1.8, // 添加人眼高度
    ...
    async selectOptimalViewPointByCamera() {
        try {
            ...
            // 获取相机当前位置
            const cameraLongitude = SuperMap3D.Math.toDegrees(cameraCartographic.longitude);
            const cameraLatitude = SuperMap3D.Math.toDegrees(cameraCartographic.latitude);
            const cameraHeight = cameraCartographic.height;
            
            console.log('相机当前位置:', { longitude: cameraLongitude, latitude: cameraLatitude, height: cameraHeight });
    ...
    async findModelPositionByPrecisePicking() {
        try {
            ...
            for (const point of samplePoints) {
                try {
                    ...
                    if (SuperMap3D.defined(pickedObject)) {
                        ...
                        console.log(`拾取到${isModel ? '模型' : '其他'}对象 (${point.strategy}):`, pickedObject.id?._name || pickedObject.primitive?.constructor?.name || '未知对象');
                        ...
                        if (isModel) {
                                    ...
                                    if (drillPickResults && drillPickResults.length > 0) {
                                        for (const drillResult of drillPickResults) {
                                            if (this.isModelObject(drillResult)) {
                                                const drillPosition = this.scene.pickPosition(windowPosition);
                                                if (drillPosition && SuperMap3D.defined(drillPosition.x)) {
                                                    modelPosition = drillPosition;
                                                    console.log('✅ drillPick获取模型位置:', modelPosition);
                                                    break;
                                                }
                                            }
                                        }
                            ...
                            // 如果获取到了有效的模型位置
                            if (modelPosition && SuperMap3D.defined(modelPosition.x)) {
                                const cartographic = SuperMap3D.Cartographic.fromCartesian(modelPosition);
                                
                                if (cartographic && !isNaN(cartographic.height) && cartographic.height > -1000) {
                                    const result = {
                                        longitude: SuperMap3D.Math.toDegrees(cartographic.longitude),
                                        latitude: SuperMap3D.Math.toDegrees(cartographic.latitude),
                                        ...
                                        hasObject: true,
                                        isModel: true,
                                        objectType: pickedObject.id?._name || pickedObject.primitive?.constructor?.name || 'model'
                                    };
                                    ...
                                    if (cartographic.height > -1000 && !isNaN(cartographic.height)) {
                                        console.log('🎯 立即使用模型表面位置，高度:', cartographic.height);
                                        return result;
                                    }
                                } else {
                            ...
                            if (modelPosition && SuperMap3D.defined(modelPosition.x)) {
                                ...
                                if (cartographic && !isNaN(cartographic.height) && cartographic.height > -1000) {
                                    const result = {
                                        ...
                                        hasObject: true,
                                        isModel: false,
                                        objectType: pickedObject.primitive?.constructor?.name || 'terrain'
                                    };
            ...
            console.log(`精确拾取找到 ${modelResults.length} 个模型位置, ${terrainResults.length} 个地形位置`);
    ...
    isModelObject(pickedObject) {
        ...
        if (pickedObject.id) {
            ...
            if (id._name && typeof id._name === 'string') {
                ...
                const modelExtensions = ['.glb', '.gltf', '.b3dm', '.i3dm', '.pnts', '.cmpt'];
            ...
            if (idConstructorName.includes('Cesium3DTile') || idConstructorName.includes('Tile')) {
    ...
    getModelBoundsFromPrimitives() {
        try {
            ...
            for (let i = 0; i < primitives.length; i++) {
                ...
                // 检查3D Tiles
                if (primitive instanceof SuperMap3D.Cesium3DTileset && primitive.boundingSphere) {
                    const boundingSphere = primitive.boundingSphere;
                    const center = boundingSphere.center;
                    
                    if (center && SuperMap3D.defined(center.x) && SuperMap3D.defined(center.y) && SuperMap3D.defined(center.z)) {
                        return {
                            center: center,
    ...
    getModelFromEntities() {
        try {
            ...
            for (const entity of entities) {
                ...
                // 检查是否有建筑物或其他几何体
                if ((entity.polygon || entity.polyline || entity.box || entity.cylinder) && entity.position) {
                    const position = entity.position.getValue(this.viewer.clock.currentTime);
                    if (position) {
                        const cartographic = SuperMap3D.Cartographic.fromCartesian(position);
                        return {
    ...
    async findModelBySampling() {
        try {
            ...
            for (const point of samplePoints) {
                try {
                    const pickedPosition = this.scene.pickPosition(new SuperMap3D.Cartesian2(point.x, point.y));
                    
                    if (pickedPosition && SuperMap3D.defined(pickedPosition.x)) {
                        // 检查这个位置是否在地面以上（可能是模型）
                        const cartographic = SuperMap3D.Cartographic.fromCartesian(pickedPosition);
                        
                        if (cartographic.height > 10) { // 高度大于10米，可能是模型
    ...
    calculateModelCenter(bounds) {
        try {
            ...
            if (magnitude < 6000000) { // 地球半径约6371000米，如果小于6000000说明是局部坐标
                console.warn('检测到局部坐标系坐标，magnitude:', magnitude, '使用相机位置作为参考');
                return this.getFallbackPosition();
            }
            ...
            if (!cartographic || isNaN(cartographic.height)) {
                try {
                    cartographic = SuperMap3D.Cartographic.fromCartesian(center, SuperMap3D.Ellipsoid.WGS84);
            ...
            // 检查转换后的坐标是否合理
            if (!cartographic || isNaN(cartographic.longitude) || isNaN(cartographic.latitude) || isNaN(cartographic.height)) {
    ...
    estimateGroundHeight(longitude, latitude) {
        try {
            // 尝试使用场景的地形提供者
            if (this.viewer.terrainProvider && this.viewer.terrainProvider.availability) {
    ...
    async selectOptimalViewPointByTerrain() {
        try {
            ...
            const centerLongitude = SuperMap3D.Math.toDegrees(centerCartographic.longitude);
            const centerLatitude = SuperMap3D.Math.toDegrees(centerCartographic.latitude);
            
            console.log('当前视野中心:', { longitude: centerLongitude, latitude: centerLatitude });
            ...
            // 检查地形提供者是否可用
            const hasTerrainProvider = this.viewer.terrainProvider && 
            ...
            for (const candidate of candidatePoints) {
                try {
                    ...
                    if (hasTerrainProvider) {
                        try {
                            const positions = [SuperMap3D.Cartographic.fromDegrees(candidate.longitude, candidate.latitude)];
                            const heights = await SuperMap3D.sampleTerrainMostDetailed(this.viewer.terrainProvider, positions);
                            
                            if (heights && heights.length > 0 && SuperMap3D.defined(heights[0].height)) {
                    ...
                    // 1. 高度分数：适中的高度更好
                    const heightScore = Math.max(0, 100 - Math.abs(observationHeight - 100));
                    score += heightScore * 0.3;
                    
                    // 2. 距离中心点的分数：不要太远也不要太近
                    const distance = SuperMap3D.Cartesian3.distance(
                        SuperMap3D.Cartesian3.fromDegrees(candidate.longitude, candidate.latitude, observationHeight),
                        SuperMap3D.Cartesian3.fromDegrees(centerLongitude, centerLatitude, terrainHeight)
                    );
                    const distanceScore = Math.max(0, 100 - Math.abs(distance - 500)); // 理想距离500米
                    score += distanceScore * 0.4;
                    
                    // 3. 地形变化分数：有一定起伏的地形更有利于观察
                    const elevationVariation = Math.abs(terrainHeight - centerCartographic.height);
    ...
    addViewPointMarker(position, name = '观察点') {
        try {
            // 移除之前的观察点标记
            const existingMarkers = this.viewer.entities.values.filter(entity => 
                entity.name && entity.name.includes('观察点')
            );
